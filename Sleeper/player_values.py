...
players_df_raw = pd.read_csv('/Users/nick/Sleeper-Dashboard/Sleeper/players-2022-04-13.csv')
...
picks_df = rosters_df.merge(draft_order_df, left_on='user_id', right_index=True)
...
def round(row):
    # return str((math.floor(row['range']/12)%5) + 1)
...
# rosters_df = rosters_df[['roster_id', 'display_name']]
#
# final_draft_order_df = final_draft_order_df.merge(rosters_df, how='left', on='roster_id')
#
...
# this_year = (year)
# next_year = (year + 1)
# year_following = (year + 2)
#
# final_draft_order_df = final_draft_order_df
# print(final_draft_order_df.info(verbose=True))
#
#
...
#         return str(row['Year']) + " " + "Pick" + " " + str(row['Round']) + "." + str(row['pick_concat'])
#
#
...
# final_draft_order_df
#
# from datetime import datetime
#
# pick_values_df = pd.merge(final_draft_order_df, pick_values_df, on='player', how='left')
# roster_values_df = roster_values_df[
#     ['display_name', 'player_id', 'full_name', 'position', 'team', 'value_1qb', 'value_2qb']]
# roster_values_df = roster_values_df.rename(
#     columns={'full_name': 'Player', 'position': 'Pos', 'display_name': 'Manager', 'team': 'Team',
#              'value_1qb': 'DyPro 1QB', 'value_2qb': 'DyPro 2QB'})
# pick_values_df = pick_values_df[['display_name', 'player', 'position', 'Team', 'value_1qb', 'value_2qb']]
# pick_values_df = pick_values_df.rename(
#     columns={'display_name': 'Manager', 'position': 'Pos', 'player': 'Player', 'value_1qb': 'DyPro 1QB',
#              'value_2qb': 'DyPro 2QB'})
#
# pick_values_df
#
...
#
# sleeper_rosters_dypro_df['DyPro 2QB']
#
