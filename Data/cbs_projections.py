...
cbs_adp_2019 = pd.read_csv('/Users/nick/Sleeper-Dashboard/Data/ADP_2019_HORSEMAN.csv')
cbs_adp_2019['Season'] = 2019
cbs_adp_2020 = pd.read_csv('/Users/nick/Sleeper-Dashboard/Data/ADP_2020_HORSEMAN.csv')
cbs_adp_2020['Season'] = 2020
cbs_adp_2021 = pd.read_csv('/Users/nick/Sleeper-Dashboard/Data/ADP_2021_HORSEMAN.csv')
cbs_adp_2021['Season'] = 2021
expert_df = pd.read_csv(f'/Users/nick/Sleeper-Dashboard/Data/expert_consensus_projections-{datetime.now():%Y-%m-%d}.csv')
df_list = [cbs_adp_2019, cbs_adp_2020, cbs_adp_2021]

for df in df_list:
    df.drop(df[df['Team'] == 'Team'].index, inplace = True)
    df.reset_index()
    df['ADP'] = df.index
    df[['Player','Team']]= df['Player'].str.split('|' , n=1,expand=True)
    df[['Player','Pos','space']]= df['Player'].str.rsplit(' ', n=2,expand=True)



    # df[['last_name','suffix']] = df['last_name'].str.split('', n=1,expand=True)
    # df['Team'] = df['Player'].str[:-3]
    # df[['Pos','junk']] = df['Pos'].str.split(" ",n=1, expand=True)

# print(df['Player'].str.split(,' 'expand=True))

cbs_adp = pd.concat([cbs_adp_2019, cbs_adp_2020, cbs_adp_2021])
cbs_adp = cbs_adp[['Player','Pos','Team','ADP']]
cbs_adp = cbs_adp.groupby(['Team']).mean().reset_index()
print(cbs_adp.sort_values(by=['ADP']))

# cbs_adp.drop(cbs_adp[cbs_adp['Team'] == 'Team'].index, inplace = True)

# cbs_adp = cbs_adp.groupby(['Team','Position'])
